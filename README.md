...
- `ENV_CONFIG`: 5도 직립 조건, 70도 종료 조건, 키 입력 시간 등
...
에피소드가 종료되거나 직립 복구가 필요하면 다음 상태 흐름을 사용합니다.

1. 현재 에피소드를 종료 상태로 처리하고 설정된 시간 동안 조작을 멈춥니다.
2. 게임오버 화면을 초기화하는 스페이스를 누릅니다.
3. 기존 코드의 에피소드 준비 스페이스를 누릅니다.
4. 목 각도가 `±5도` 이내로 돌아올 때까지 기다립니다.
5. 직립이 확인되면 시작 스페이스를 한 번 더 눌러 다음 에피소드를 시작합니다.

각도 검출이 연속으로 실패하거나 종료 각도를 넘은 자세가 계속 보이는 경우에도
현재 상황을 에피소드 종료로 처리하고 위의 일반적인 다음 에피소드 준비 순서를
그대로 실행합니다. 준비 순서가 끝난 시점부터 `upright_timeout` 시간이 새로 보장됩니다.
...
복구가 실행되면 콘솔에 `[직립 복구]` 상태가 출력됩니다.

### 방향 행동 보상

목 각도를 0도 방향으로 되돌리는 키 입력에 추가 보상을 줍니다. 현재 설정값은
올바른 행동 `+1.0`, 잘못된 행동 `-2.0`입니다. 중심 근처에서는 방향키를
계속 누르지 않도록 무행동을 올바른 행동으로 처리합니다.

| 현재 목 각도 | 올바른 행동 | 행동 번호 | 방향 보상 |
|---|---|---:|---:|
| `+10도` 초과 | 왼쪽 방향키 | `0` | `+1.0` |
| `-10도` 미만 | 오른쪽 방향키 | `1` | `+1.0` |
| `±10도` 이내 | 아무것도 하지 않음 | `2` | `+1.0` |

위 상태에서 다른 행동을 선택하면 `-2.0`이 적용됩니다. 따라서 무행동을 10도 밖에서
...
현재 이어서 학습 동작은 다음과 같습니다.

- 모델 내부의 `num_timesteps`는 유지됩니다.
- `total_timesteps`는 최종 목표 누적값이 아니라 이번 실행에서 추가로 학습할 스텝 수입니다.
  예를 들어 기존 모델이 100,000 스텝이고 `total_timesteps=100_000`이면 약 200,000
  누적 스텝까지 학습합니다.
- `max_episodes`는 이번 실행에서 진행한 에피소드만 계산하며, 이전 실행의 에피소드 수는
  더하지 않습니다.
- 실행 폴더, Monitor 로그와 체크포인트 저장 주기는 새 실행 기준으로 다시 시작합니다.
- PPO는 기존 정책과 가치망을 이어서 사용합니다.
- DQN은 정책망과 누적 스텝을 이어서 사용하지만, 현재 코드는 저장된 replay buffer를
  자동으로 다시 불러오지는 않습니다.
