...
행동은 `0=왼쪽 방향키`, `1=오른쪽 방향키`입니다.

모델 초기화 시간 동안 먼저 시작된 게임은 학습에 포함하지 않습니다. 최초 게임은
방향키 입력 없이 60도 초과까지 추적한 후, 설정된 대기 시간과 재시작 절차를 거쳐
...
- `upright_fallen_retry_seconds`: 직립 대기 중 종료 각도(기본 60도)를 넘은 자세가 몇 초간 지속되면 복구할지 설정
- `upright_fallen_max_retries`: 넘어진 자세에 대한 스페이스 복구 최대 횟수
- `upright_fallen_max_retries=0`: 넘어진 자세 자동 복구 비활성화

각도가 정상 검출된 `5~60도` 구간에서는 아직 직립 상태로 돌아오는 중인 것으로 보고
복구용 스페이스를 누르지 않습니다. 하지만 60도를 넘은 자세가 설정 시간 동안
...
목 각도를 0도 방향으로 되돌리는 키 입력에 추가 보상을 줍니다. 현재 기본값은
올바른 방향 `+1.0`, 반대 방향 `-0.5`입니다.
...
올바른 방향과 반대로 누르면 `-0.5`가 적용됩니다. 아래 설정으로 보상 강도와
